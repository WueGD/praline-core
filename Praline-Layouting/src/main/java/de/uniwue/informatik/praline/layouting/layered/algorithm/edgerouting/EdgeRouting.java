...
public class EdgeRouting {
    ...
    public void routeEdges () {
        ...
        for (int rank = 0; rank < (cmResult.getNodeOrder().size() - 1); rank++) {
            ...
            int maxLRLevel = handleLREdges(rank, edgeToLayerLR, outlineContourLR, conflicts);
            ...
            handleRLEdges(rank, edgeToLayer, outlineContourRL);

            // place lr-edges relative to rl-edges
            int maxLevel = placeEdgesFinally(edgeToLayerLR, outlineContourLR, maxLRLevel, edgeToLayer, outlineContourRL);
    ...
    private int handleLREdges(int rank, Map<Edge, Integer> edgeToLayerLR, LinkedList<ContourPoint> outlineContourLR, Map<Edge, Edge> conflicts) {
        ...
        return activeCandidates.size() - 1;
    }

    private void handleRLEdges(int rank, Map<Edge, Integer> edgeToLayer, LinkedList<ContourPoint> outlineContourRL) {
        ArrayList<Double> activeCandidates = new ArrayList<>();
        LinkedList<ContourPoint> lastPositions = new LinkedList<>();
    ...
    private int placeEdgesFinally(Map<Edge, Integer> edgeToLayerLR, LinkedList<ContourPoint> outlineContourLR, int maxLRLevel, Map<Edge, Integer> edgeToLayer, LinkedList<ContourPoint> outlineContourRL) {
